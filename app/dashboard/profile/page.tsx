...
export default function ProfilePage() {
  ...
  const handlePhotoSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Force correct public URL format
  ...
  const accentBg = isISAP ? 'bg-red-50 dark:bg-red-950/30' : 'bg-blue-50 dark:bg-blue-950/30'
  const accentText = isISAP ? 'text-red-600 dark:text-red-400' : 'text-blue-600 dark:text-blue-400'
  const accentBorder = isISAP ? 'border-red-200 dark:border-red-800' : 'border-blue-200 dark:border-blue-800'
        ...
        <div className="px-4 sm:px-6 pb-6">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 -mt-12 sm:-mt-16">
            ...
            <button
              onClick={() => fileRef.current?.click()}
              disabled={uploading}
              className={`sm:hidden flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold border-2 transition-all ${accentBorder} ${accentBg} ${accentText}`}
            >
              <Camera size={15} />
      ...
      >
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Update your display name and year level
          </p>
        </div>
        ...
        <div className={`rounded-xl border p-4 ${accentBg} ${accentBorder}`}>
          <div className="flex items-start gap-3">
            <Camera size={16} className={`${accentText} shrink-0 mt-0.5`} />
            <div>
              <p className={`text-xs font-semibold ${accentText} mb-1`}>
                Profile Photo
              </p>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Click the camera icon on your photo above to upload. Your photo will appear on your ID card. Max 5MB.
              </p>
              <button
                onClick={() => fileRef.current?.click()}
                disabled={uploading}
                className="mt-2 text-xs font-semibold px-3 py-1.5 rounded-lg text-white transition-all"
                style={{ backgroundColor: accentColor, opacity: uploading ? 0.7 : 1 }}
              >
        ...
        {/* Success */}
        {success && (
          <div className="bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 rounded-xl px-4 py-3 flex items-center gap-2">
